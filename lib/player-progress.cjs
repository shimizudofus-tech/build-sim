...
function applyEnergyRegen(progress, now = Date.now()) {
  const stored = normalizeStoredProgress(progress);
  if (stored.energy >= stored.energyCap) return stored;
  const elapsed = Math.max(0, now - stored.energyUpdatedAt);
  const ticks = Math.floor(elapsed / ENERGY_REGEN_MS);
  if (ticks <= 0) return stored;
  stored.energy = Math.min(stored.energyCap, stored.energy + ticks);
  stored.energyUpdatedAt += ticks * ENERGY_REGEN_MS;
  return stored;
}

function msUntilNextEnergy(progress, now = Date.now()) {
  const stored = applyEnergyRegen(progress, now);
  if (stored.energy >= stored.energyCap) return 0;
  const elapsed = Math.max(0, now - stored.energyUpdatedAt);
  return Math.max(0, ENERGY_REGEN_MS - elapsed);
}
