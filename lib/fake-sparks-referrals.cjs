/**
 * KRAPS — parrainage / invitations (max 3, +10 sparks par filleul validé).
 * Stockage : db.referrals[], db.users[userId].pendingReferrerId, champs sur fakeSparksUsers.
 */

const { userLinkStatus } = require("./auth-accounts.cjs");
...
function tryValidateReferralForUser(db, referredUser) {
  ...
  const referrerId = normalizeReferrerId(referredUser.pendingReferrerId);
  if (!referrerId) return { validated: false };
  ...
  const referrerUser = db.users[referrerId];
  const now = Date.now();
  const referral = {
    id: randomReferralId(),
