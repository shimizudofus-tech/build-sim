...
/** Récompenses de run désactivées (classements uniquement). */
...
function getSkillSurvivorState(db, user) {
  ...
  const canStartFresh = Boolean(user && eligible && !activeRun);
  const freeAvailable = Boolean(user && eligible);
...
/** Fusionne une écriture Skill Survivor dans le snapshot KV le plus récent (évite les lost updates). */
function mergePersistedSkillSurvivorWrite(intoDb, fromDb) {
  if (!intoDb || !fromDb) return;
  ensureSkillSurvivorDb(intoDb);
  ensureSkillSurvivorDb(fromDb);
  const intoStore = intoDb.skillSurvivor;
  const fromStore = fromDb.skillSurvivor;
  for (const [id, run] of Object.entries(fromStore.runs || {})) {
    if (!run || typeof run !== "object") continue;
    const prev = intoStore.runs[id];
    if (!prev || (run.updatedAt || 0) >= (prev.updatedAt || 0)) {
      intoStore.runs[id] = run;
    }
  }
  for (const [uid, stats] of Object.entries(fromStore.users || {})) {
    if (!stats || typeof stats !== "object") continue;
    const prev = intoStore.users[uid];
    if (!prev || (stats.updatedAt || 0) >= (prev.updatedAt || 0)) {
      intoStore.users[uid] = stats;
    }
  }
  if (!fromDb.fakeSparksUsers) return;
  intoDb.fakeSparksUsers = normalizeFakeSparksUsers(intoDb.fakeSparksUsers);
  const fromSparks = normalizeFakeSparksUsers(fromDb.fakeSparksUsers);
  for (const [uid, row] of Object.entries(fromSparks)) {
    if (!row) continue;
    const prev = intoDb.fakeSparksUsers[uid];
    if (!prev || (row.updated_at || 0) >= (prev.updated_at || 0)) {
      intoDb.fakeSparksUsers[uid] = row;
    }
  }
}
